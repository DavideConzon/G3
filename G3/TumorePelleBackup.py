...
"""
#FUNZIONE DI CALCOLO DELLA TOP 2 CATEGORICAL ACCURACY
def top_2(y_true, y_pred):
    return top_k_categorical_accuracy(y_true, y_pred, k=2)

#CARICAMENDO DEL PATH DEL MODELLO  ADDESTRATO PER LA CLASSIFICAZIONE 
MODEL_PATH = r"C:\Users\david\OneDrive\Desktop\G3\skin_cancer_best_model.h5"

#DEVIIZIONE DEI RUOLI DEGLI UTENTI
...
#INTERFACCIE E FUNZIONI DEGLI UTENTI
cap = None
camera_label = None
...
def register_callback(nome_entry, cognome_entry, ruolo_combobox, email_entry, telefono_entry, password_entry, feedback_label):
    ...
    c = conn.cursor()
    c.execute('''
    INSERT INTO users (nome, cognome, ruolo, email, telefono, password)
    VALUES (?, ?, ?, ?, ?, ?)
    ''', (nome, cognome, ruolo, email, telefono, hashed_password))
    conn.commit()
    conn.close()
...
def login_callback(email_entry, password_entry):
    ...
    if user:
        messagebox.showinfo("Benvenuto", f"Ciao {user[1]} {user[2]}! Sei loggato come {user[3]}")
        # Continue to next step after successful login (e.g., show patient info, dashboard, etc.)
    else:
        messagebox.showerror("Errore", "Credenziali non valide")

# Setup main window
root = Tk()
root.title("Gestione Pazienti")
...
#AVVIO DI UNA SESSIONE FLASK PER LA GESTIONE DEI VARI RUOLI

app = Flask(__name__)

def run_flask():
    app.run(debug=True, use_reloader=False)  # Disabling reloader when running in a thread.

flask_thread = threading.Thread(target=run_flask)
flask_thread.start()


@app.route('/search_paziente', methods=['POST'])
def search_paziente_route():
    nome_paziente = request.form.get('nome_paziente')
    results = search_paziente(nome_paziente)
    if results:
        return jsonify({'success': True, 'pazienti': results})
    else:
...
# Define the search_patient function
def search_patient(patient_name, user_role, user_name):
    # Logic for searching a patient (you may want to query a database or CSV file)
    print(f"Searching for patient: {patient_name} by {user_name} with role {user_role}")
    # Example: You can fetch the patient data and update the UI accordingly
    pass

# Inside your Tkinter Button definition
search_button = tk.Button(medico_admin_frame, text="Cerca", command=lambda: search_patient(search_entry.get(), user_role, user_name), bg='#0078D7', fg='#ffffff')
search_button.pack()


def search_paziente(nome_paziente):
    results = []
    for paziente in pazienti:
        if nome_paziente.lower() in paziente['nome'].lower():  # Supponendo che il campo si chiami 'nome'
            results.append(paziente)
    return results

def search_utente(username):
    for utente in utenti:
        if username == utente['username']:  # Supponendo che il campo si chiami 'username'
            return utente
    return None
"""
